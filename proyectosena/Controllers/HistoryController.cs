    ...
    public class HistoryController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateHistory([FromBody] History history)
        {
            try
            {
                ...
                if (history.IdRequest == Guid.Empty || history.IdUser == Guid.Empty)
                    return BadRequest("History must have a valid IdRequest and IdUser.");

                if (string.IsNullOrWhiteSpace(history.NewStatus))
                    return BadRequest("NewStatus is required.");

                var newHistory = await _historyRepository.Create(history);
                return Ok(newHistory);
            }
            catch
            {
                return StatusCode(StatusCodes.Status500InternalServerError, "Error creating the history record.");
            }
        }

        // -------------------- PUT: api/history/UpdateHistory --------------------
        ...
        public async Task<IActionResult> UpdateHistory([FromBody] History history)
        {
            try
            {
                ...
                // El IdHistory es obligatorio para identificar el registro a actualizar
                if (history.IdHistory == Guid.Empty)
                    return BadRequest("IdHistory is required to update a record.");

                // Verifica si existe antes de intentar actualizar
                var exists = await _historyRepository.Exists(history.IdHistory);
                if (!exists)
                    return NotFound("The history record does not exist.");

                var updated = await _historyRepository.Update(history);

                if (updated == null)
                    return BadRequest("Could not update the history record.");

                return Ok(updated);
            }
            catch
            {
                return StatusCode(StatusCodes.Status500InternalServerError, "Error updating the history record.");
            }
        }
        ...
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> DeleteHistory(Guid idHistory)
        {
            try
            {
                var deleted = await _historyRepository.Delete(idHistory);

                // Retorna false si el registro no existe en la base de datos
                if (!deleted)
                    return BadRequest("Could not delete the record. Please verify it exists.");

                return Ok("History record deleted successfully.");
            }
            catch
            {
                return StatusCode(StatusCodes.Status500InternalServerError, "Error deleting the history record.");
            }
        }
